...
function getDates() {
  ...
  const end = new Date(y, m - 1 + MONTHS_AHEAD, d);
...
async function isHoliday(calendar, dateStr) {
  const [year, month, day] = dateStr.split('-').map(Number);
  const dayStart = new Date(Date.UTC(year, month - 1, day, -9, 0, 0));
  const dayEnd   = new Date(Date.UTC(year, month - 1, day, 15, 0, 0));
  try {
    const res = await calendar.events.list({
...
export async function getAvailableSlots(teacherId) {
  ...
  for (const dateStr of dates) {
    ...
    // 祝日チェック
    const holiday = await isHoliday(calendar, dateStr);

    // 祝日は日曜と同じ時間（10:00〜19:00）
    let timeRange;
    if (holiday) {
