...
export async function getAvailableSlots(teacherId) {
  ...
  const minUtc = new Date(nowUtc.getTime() + 2 * 60 * 60 * 1000);
...
export async function createBooking({ teacherId, date, startTime, endTime, studentName, parentName, email, notes }) {s
  const teacher = TEACHERS.find(t => t.id === teacherId);
